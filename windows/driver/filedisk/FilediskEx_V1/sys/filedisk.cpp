...
/*    for (n = 0, n_created_devices = 0; n < n_devices; n++)
    {
        status = FileDiskCreateDataDevice(DriverObject, n, FILE_DEVICE_DISK);
    ...
    {
        status = FileDiskCreateDataDevice(DriverObject, n, FILE_DEVICE_CD_ROM);
...
FileDiskCreateControlDevice ( IN PDRIVER_OBJECT   DriverObject )
{
    ...
    RtlZeroMemory(device_extension->union_special_dev_ext.ctrl_dev_ext.dev_map,sizeof(DEVICE_EXTENSION::CTRL_DEV_EXT::DATA_DEV_MAP_ITEM)*DATA_DEV_MAP_SIZE);

    KeInitializeSpinLock(&device_extension->union_special_dev_ext.ctrl_dev_ext.dev_map_lock);
    ...
    )
{
    ...
    else
    {
        ...
        default:
            {
                KdPrint((
                    "FileDisk Data: Not handled device request: IoControlCode %#x\n",
                    io_stack->Parameters.DeviceIoControl.IoControlCode
                    ));
...
)
{
    ...
    SetDataDeviceMap(CtrlDevObj, mount_information->DriveLetter, DataDevObj);
...
)
{
    ...
    SetDataDeviceMap(CtrlDevObj, *pDriverLetter, NULL);
...
FindDataDeviceByDriverLetter(PDEVICE_OBJECT CtrlDevObj, CHAR DriverLetter, PDEVICE_OBJECT * pDataDevObj)
{
    ...
    if(DriverLetter >= 'a' && DriverLetter <= 'z' || DriverLetter >= 'A' && DriverLetter <= 'Z' )
    {
        ...
        KeAcquireSpinLock(&CtrlDevObjExt->union_special_dev_ext.ctrl_dev_ext.dev_map_lock,&irql);

        for(int i = 0; i < DATA_DEV_MAP_SIZE; ++i)
        {
            CHAR tmpDL = CtrlDevObjExt->union_special_dev_ext.ctrl_dev_ext.dev_map[i].driver_letter;
            ...
            {
                if (NULL != CtrlDevObjExt->union_special_dev_ext.ctrl_dev_ext.dev_map[i].data_dev_obj) 
                {
                    *pDataDevObj = CtrlDevObjExt->union_special_dev_ext.ctrl_dev_ext.dev_map[i].data_dev_obj;
        ...
        KeReleaseSpinLock(&CtrlDevObjExt->union_special_dev_ext.ctrl_dev_ext.dev_map_lock,irql);
    }
    ...
    if(DriverLetter >= 'a' && DriverLetter <= 'z' || DriverLetter >= 'A' && DriverLetter <= 'Z' )
    {
        ...
        if( NULL == DataDevObj )
        {
            for(int i = 0; i < DATA_DEV_MAP_SIZE; ++i)
            {
                ...
                {
                        CtrlDevObjExt->union_special_dev_ext.ctrl_dev_ext.dev_map[i].data_dev_obj = DataDevObj;
                        
                        CtrlDevObjExt->union_special_dev_ext.ctrl_dev_ext.dev_map[i].driver_letter = 0;
        ...
        else
        {
            for(int i = 0; i < DATA_DEV_MAP_SIZE; ++i)
            {
                if( 0 == CtrlDevObjExt->union_special_dev_ext.ctrl_dev_ext.dev_map[i].driver_letter )
                {
                        CtrlDevObjExt->union_special_dev_ext.ctrl_dev_ext.dev_map[i].data_dev_obj = DataDevObj;
                        
                        CtrlDevObjExt->union_special_dev_ext.ctrl_dev_ext.dev_map[i].driver_letter = DriverLetter;
