    ...
    )
{
    ...
    device_object->Flags |= ( DO_BUFFERED_IO | DO_DIRECT_IO );
    ...
    for (;;)
    {
            ...
            ))
        {
            ...
            case IRP_MJ_READ:
                ...
                {
                    KdPrint(("IRP_MJ_READ IRP_BUFFERED_IO !!!!!!\n"));

                    KdPrint(("IRP_MJ_READ Read.Length = %x, Read.ByteOffset = %x\n", io_stack->Parameters.Read.Length,io_stack->Parameters.Read.ByteOffset.QuadPart));
                    irp->IoStatus.Status = ZwReadFile(device_extension->file_handle,NULL,NULL,NULL,&(irp->IoStatus),irp->AssociatedIrp.SystemBuffer,io_stack->Parameters.Read.Length,&(io_stack->Parameters.Read.ByteOffset),NULL);
                    KdPrint(("IRP_MJ_READ irp->IoStatus.Status = %x, irp->IoStatus.Information = %x\n",irp->IoStatus.Status,irp->IoStatus.Information));
                }
                else
                {
                    KdPrint(("IRP_MJ_READ NOT IRP_BUFFERED_IO !!!!!!\n"));
                    ...
                    KdPrint(("IRP_MJ_READ mdl_address:0x%.8x\n",mdl_address));
                    KdPrint(("IRP_MJ_READ mdl_length:%d\n",mdl_length));
                    KdPrint(("IRP_MJ_READ mdl_offset:%d\n",mdl_offset));

                    if (mdl_length!=io_stack->Parameters.Read.Length)
                    {
                        //MDL的长度应该和读长度相等
                        KdPrint(("!!!!!! WARNING IRP_MJ_READ mdl_length(0x%.8x)!= Read.Length(0x%.8x) !!!\n",mdl_length,io_stack->Parameters.Read.Length));
                        if( mdl_length < io_stack->Parameters.Read.Length ) 
                        {
                            KdPrint(("!!!!!! ERROR IRP_MJ_READ mdl_length(0x%.8x)< Read.Length(0x%.8x) !!!\n",mdl_length,io_stack->Parameters.Read.Length));
                            irp->IoStatus.Information = 0;
                    ...
                    KdPrint(("IRP_MJ_READ kernel_address:0x%.8x\n",kernel_address));

                    irp->IoStatus.Status = ZwReadFile(device_extension->file_handle,NULL,NULL,NULL,&(irp->IoStatus),kernel_address,io_stack->Parameters.Read.Length,&(io_stack->Parameters.Read.ByteOffset),NULL);
                ...
                {
                    KdPrint(("!!!!!! ERROR IRP_MJ_READ irp->IoStatus.Status = %x, irp->IoStatus.Information = %x\n",irp->IoStatus.Status,irp->IoStatus.Information));
                }
                break;
            ...
            case IRP_MJ_WRITE:
                if(irp->Flags & IRP_BUFFERED_IO)
                {
                    KdPrint(("IRP_MJ_WRITE IRP_BUFFERED_IO !!!!!!\n"));

                    KdPrint(("IRP_MJ_WRITE Write.Length = %x, Write.ByteOffset = %x\n", io_stack->Parameters.Write.Length,io_stack->Parameters.Write.ByteOffset.QuadPart));
                    irp->IoStatus.Status = ZwWriteFile(device_extension->file_handle,NULL,NULL,NULL,&(irp->IoStatus),irp->AssociatedIrp.SystemBuffer,io_stack->Parameters.Write.Length,&(io_stack->Parameters.Write.ByteOffset),NULL);
                    KdPrint(("IRP_MJ_WRITE irp->IoStatus.Status = %x, irp->IoStatus.Information = %x\n",irp->IoStatus.Status,irp->IoStatus.Information));
                }
                else
                {
                    KdPrint(("IRP_MJ_WRITE NOT IRP_BUFFERED_IO !!!!!!\n"));
                    mdl_length = MmGetMdlByteCount(irp->MdlAddress);
                    mdl_address = MmGetMdlVirtualAddress(irp->MdlAddress);
                    mdl_offset = MmGetMdlByteOffset(irp->MdlAddress);

                    KdPrint(("IRP_MJ_WRITE mdl_address:0x%.8x\n",mdl_address));
                    KdPrint(("IRP_MJ_WRITE mdl_length:%d\n",mdl_length));
                    KdPrint(("IRP_MJ_WRITE mdl_offset:%d\n",mdl_offset));

                    if (mdl_length!=io_stack->Parameters.Write.Length)
                    {
                        //MDL的长度应该和写长度相等
                        KdPrint(("!!!!!! WARNING IRP_MJ_WRITE mdl_length(0x%.8x)!= Write.Length(0x%.8x) !!!\n",mdl_length,io_stack->Parameters.Write.Length));
                        if( mdl_length < io_stack->Parameters.Write.Length ) 
                        {
                            KdPrint(("!!!!!! ERROR IRP_MJ_WRITE mdl_length(0x%.8x)< Write.Length(0x%.8x) !!!\n",mdl_length,io_stack->Parameters.Write.Length));
                            irp->IoStatus.Information = 0;
                            irp->IoStatus.Status = STATUS_UNSUCCESSFUL;
                            break;
                        }
                    ...
                    kernel_address = MmGetSystemAddressForMdlSafe(irp->MdlAddress,NormalPagePriority);
                    KdPrint(("IRP_MJ_WRITE kernel_address:0x%.8x\n",kernel_address));
                    irp->IoStatus.Status = ZwWriteFile(device_extension->file_handle,NULL,NULL,NULL,&(irp->IoStatus),kernel_address,io_stack->Parameters.Write.Length,&(io_stack->Parameters.Write.ByteOffset),NULL);
                    KdPrint(("IRP_MJ_WRITE irp->IoStatus.Status = %x, irp->IoStatus.Information = %x\n",irp->IoStatus.Status,irp->IoStatus.Information));
                }
                if(!NT_SUCCESS(irp->IoStatus.Status))
                {
                    KdPrint(("!!!!!! ERROR IRP_MJ_WRITE irp->IoStatus.Status = %x, irp->IoStatus.Information = %x\n",irp->IoStatus.Status,irp->IoStatus.Information));
                }
                break;
...
*/
	...
	DbgPrint("DeviceObject->AlignmentRequirement : %lu\n",DeviceObject->AlignmentRequirement);
