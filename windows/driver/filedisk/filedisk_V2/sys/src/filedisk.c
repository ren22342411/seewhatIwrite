    ...
    )
{
    ...
    {       
        ...
        if(!NT_SUCCESS(Irp->IoStatus.Status))
        {
            ...
            KdPrint(("IRQL = 0x%.8x, FILE HANDLE = 0x%.8x\n",Irql, device_extension->file_handle));

            KdPrint(("Length = %x, ByteOffset = %x\n", 
                     IRP_MJ_READ == io_stack->MajorFunction 
            ...
            KdPrint(("!!!!!! ERROR Irp->IoStatus.Status = %x, Irp->IoStatus.Information = %x\n",Irp->IoStatus.Status,Irp->IoStatus.Information));

            // Can not be handled, so deliver this IRP to the Thread and pend it.
        ...
        {
            IRP_MJ_READ == io_stack->MajorFunction
                ? KdPrint(("Successful IRP_MJ_READ\n"))
                : KdPrint(("Successful IRP_MJ_WRITE\n"));
        }
    ...
    )
{
    ...
    for (;;)
    {
            ...
            ))
        {
            ...
            case IRP_MJ_READ: 
                if(irp->Flags & IRP_BUFFERED_IO) // If the IRP_MJ_READ Buffered IO can not be handled synchronously in Dispatch Function, it will be handled here. 
                {
                    ...
                    if(!NT_SUCCESS(irp->IoStatus.Status))
                    {
                        KdPrint(("!!!!!! ERROR in Thread Read irp->IoStatus.Status = %x, irp->IoStatus.Information = %x\n",irp->IoStatus.Status,irp->IoStatus.Information));
                    }
                    break;
                }
                // else continue to "case IRP_MJ_WRITE:" Dirrect IO of IRP_MJ_READ and IRP_MJ_WRITE will be handled asynchronously here.
            case IRP_MJ_WRITE:
                {
                    ...
                    if (mdl_length != parameter_length)
                    {
                        if( mdl_length < parameter_length ) 
                        {
                            KdPrint(("!!!!!! ERROR mdl_length(0x%.8x)< parameter_length(0x%.8x) \n",mdl_length,parameter_length));
                            irp->IoStatus.Information = 0;
                    ...
                    irp->IoStatus.Status = IRP_MJ_READ == io_stack->MajorFunction 
                        ? ZwReadFile(device_extension->file_handle,NULL,NULL,NULL,&(irp->IoStatus),kernel_address,parameter_length,&parameter_offset,NULL)
                        : ZwWriteFile(device_extension->file_handle,NULL,NULL,NULL,&(irp->IoStatus),kernel_address,parameter_length,&parameter_offset,NULL) ;
                    ...
                    {
                        KdPrint(("Success Thread Handling, FILE HANDLE = 0x%.8x\n",device_extension->file_handle));
                    }
                }
                break;
