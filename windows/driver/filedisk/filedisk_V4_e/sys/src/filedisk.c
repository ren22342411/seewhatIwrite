    ...
    )
{
    ...
    device_object->Flags |= ( DO_BUFFERED_IO | DO_DIRECT_IO | DO_FORCE_NEITHER_IO );
    ...
    )
{
    ...
    else if(Irp->Flags & IRP_NOCACHE ) 
    {
        if ( Irp->Flags & IRP_PAGING_IO)
        {
            if(Irp->Flags & IRP_NOCACHE) 
            {
                KdPrint(("Direct IO, IRP_NOCACHE \t"));
            }
    
            if(Irp->Flags & IRP_PAGING_IO) 
            {
                KdPrint(("Direct IO, IRP_PAGING_IO \t"));
            }
        ...
        else
        {
            ...
            KdPrint(("Irp->Flags & IRP_NOCACHE\n"));

            if(NULL == Irp->MdlAddress)
            {
                KdPrint(("NULL == Irp->MdlAddress\n"));
            }
    
            if(NULL == Irp->AssociatedIrp.SystemBuffer)
            {
                KdPrint(("NULL == Irp->AssociatedIrp.SystemBuffer\n"));
            }
            ...
            __try
            {
                ...
                if( IRP_MJ_READ == io_stack->MajorFunction )
                {
                    ProbeForWrite(Irp->UserBuffer,io_stack->Parameters.Read.Length,4);
                    Irp->IoStatus.Status = ZwReadFile(device_extension->file_handle,NULL,NULL,NULL,&(Irp->IoStatus),Irp->UserBuffer,io_stack->Parameters.Read.Length,&(io_stack->Parameters.Read.ByteOffset),NULL);
                }
                else
                {
                    ASSERT((IRP_MJ_WRITE == io_stack->MajorFunction));
                    ProbeForRead(Irp->UserBuffer,io_stack->Parameters.Write.Length,4);
                    Irp->IoStatus.Status = ZwWriteFile(device_extension->file_handle,NULL,NULL,NULL,&(Irp->IoStatus),Irp->UserBuffer,io_stack->Parameters.Write.Length,&(io_stack->Parameters.Write.ByteOffset),NULL);
                }
