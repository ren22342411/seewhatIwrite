// Test10.cpp : Defines the entry point for the application.
//
...
public:
	...
	virtual void OnPaint()
	{
		PAINTSTRUCT ps;
		HDC hdc = BeginPaint(m_hWnd, &ps);
		::TextOut(hdc,0,0,L"654321",6);
		EndPaint(m_hWnd, &ps);

	}
};

class CNewWnd2 : public CMyWnd 
{
public:
...
CMyWnd* FindCMyWnd(HWND hWnd)
{
	...
	{
		maphwnd2wnd.insert(make_pair(hWnd,tmpwnd));
		tmpwnd->m_hWnd = hWnd;
		return tmpwnd;
	}
	return NULL;
